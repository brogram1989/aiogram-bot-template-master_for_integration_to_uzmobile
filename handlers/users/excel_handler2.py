...
async def handle_document(message: types.Message, state:FSMContext):
    global excel_data, soni, bugun  # Declare the global variable
    ...
    try:
        ...
        #excel_data["Доставка на объект"] = pd.to_datetime(excel_data["Доставка на объект"], errors="coerce").dt.strftime("%d-%m-%Y")
        #bo'lar edi
...
async def todays_launch_plan(message: Message):
    try:
        ...
        # Convert the "Запуск в процессе" column to datetime if it's not already in datetime format

        excel_data["Запуск в процессе"] = pd.to_datetime(excel_data["Запуск в процессе"], errors="coerce").dt.date


        # Filter the DataFrame for rows where the "Запуск в процессе" column equals today_date
        filtered_rows = excel_data[excel_data["Запуск в процессе"] == bugun]
        ...
        if not filtered_rows.empty:
            ...
            for index, row in filtered_rows.iterrows():
                ...
                if row["Online\\New sites"] in ["New Site","New site","new site", "NEW SITE"]:
                    ...
                    i += 1
                    spiska_zapusk = spiska_zapusk + f"<b>{i}. {bts_id_zapusk} </b> {bts_name}  {bts_texnologiya}\n\n"
                elif row["Online\\New sites"] in ["Online", "online", "ONLINE"]:
                    bts_id_modern = row["BTSID"]
                    j += 1
                    spiska_modern = spiska_modern + f"<b>{j}. {bts_id_modern} </b> {bts_name}  {bts_old_texnologiya} ⏩🆕  {bts_texnologiya} \n\n"

            await message.answer(f"<b>Bugun, {today_date_str} kuni yangi ishga tushirilishi rejalashtirilgan BS(lar) soni {i} ta:</b>\n"
                                 f"{spiska_zapusk}\n"
                                 f"<b>Shuningdek, quyidagi {j} ta BS(lar)da SWAP va modernizatsiya ishlari rejalashtirilgan:</b>\n"
                                 f"{spiska_modern}\n",
                                 reply_markup=None)
        else:
...
async def launched_by(message: Message, state:FSMContext):
    ...
    await message.answer("Qaysi vaqt oralig'ida ishga tushgan obektlarni ko'rmoqchisiz? \n"
                         ...
                         "<code>10-01-2024:31-03-2024</code> ko'rinishida.\n"
                         "kiritilgan ikki sanani '<code>:</code>' orqali ajrating. "
                         "Qidirayotgan oralliqni kirgazishingizda yuqoridagi namuna singari, hech qanday probelsiz kiritishingiz muhum!", reply_markup=None)
...
async def choosen_time_shift(message: Message, state: FSMContext):
    try:
        ...
        if not filtered_df.empty:
            ...
            new = 0
            moder = 0
            filtered_df['Запуск'] = filtered_df['Запуск'].dt.strftime("%d-%m-%Y")
    ...
    try:
        ...
        if not excel_data.empty:
            ...
            for index, row in excel_data.iterrows():
                ...
                bts_old_texnologiya = row["Существующая технология"]
                bts_texnologiya = row["Технология после модернизации"]
                if row["Монтаж заверщен"] == bugun:
                    x += 1
                    montaj_zavershyon += f"<b>{x}. {bts_id} </b> {bts_name}  {bts_old_texnologiya} ⏩🆕  {bts_texnologiya}\n"
                elif row["Монтаж в процессе"] == bugun:
                    j += 1
                    montaj_nachat += f"<b>{j}. {bts_id} </b> {bts_name}  {bts_old_texnologiya} ⏩🆕  {bts_texnologiya}\n"
                elif row["Доставка на объект"] == bugun:
                    i += 1
                    dostavka_vobekt += f"<b>{i}. {bts_id} </b> {bts_name}  {bts_old_texnologiya} ⏩🆕  {bts_texnologiya}\n"
        ...
        if not excel_data.empty:
            ...
            for index, row in excel_data.iterrows():
                ...
                if pd.notna(row["Монтаж заверщен"]) and pd.isna(row["Запуск"]):
                    ...
                    x += 1
                    ne_zapushen += f"<b>{x}. {bts_id} </b> {bts_name}  {day_difference} kundan beri. Izoh: {koment1} {koment2}\n"

            await message.answer(f"<b>quyidagi {x} ta obektlarda montaj ishlari yakunlangan, lekin zapusk bo'lmagan:</b>\n"
                                 f"{ne_zapushen}\n",
                                 reply_markup=None)
        else:
